...
from cap_cls_dataset import ImageFolderDataset
...
def main():
    ...
    tokenizer = OFATokenizer.from_pretrained(args.cap_dir)

    txt = " what does the image describe?"
    cap_inputs = tokenizer([txt], return_tensors="pt").input_ids
    folder_path=args.folder_path
    dataset = ImageFolderDataset(folder_path, transform=patch_resize_transform)
    ...
    annot_path=(args.annot_path +'/'+ args.folder_path.split('/')[1]+'.txt')
    print(annot_path)
    ...
    for batch_idx, image in enumerate(dataloader):    
        ...
        if args.pred_num==1:
            ...
            topN_predicted_class_idx=logits.argsort(descending=True)[:, :int(args.pred_num)].squeeze()
            ...
            for i in range(int(args.pred_num)):
